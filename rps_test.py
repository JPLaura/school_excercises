...
def play():
    p_choice = input("What do you choose? (rock, paper, scissors").capitalize()
...
def game_start():
    begin = input("Would you like to play Rock, Paper, Scissors? (yes/no) ").capitalize()
    while begin != "Yes":
        if begin == "No":
            print("Game Over")
            sys.exit()
    ...
    play()
    while True:
        begin = input('Play again? (yes/no)' .capitalize()
        while begin != "Yes":
            if begin == "No":
                print("Game Over")
            else:
